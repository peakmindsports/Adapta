...
type UploadKey = "dictamen" | "programacion" | "criterios" | "unidades" | "material" | "proyecto" | "project_criteria" | "project_math" | "project_language" | "project_science" | "project_english";
...
export default function Home() {
  ...
  const [files, setFiles] = useState<Record<UploadKey, File[]>>({ dictamen: [], programacion: [], criterios: [], unidades: [], material: [], proyecto: [], project_criteria: [], project_math: [], project_language: [], project_science: [], project_english: [] });
  ...
  const generate = async (kind: "adaptation" | "project") => {
    ...
    const groups: [UploadKey, File[]][] = kind === "adaptation" ? [["dictamen", files.dictamen], ["programacion", files.programacion], ["criterios", files.criterios], ["unidades", files.unidades], ["material", files.material]] : [["project_criteria", files.project_criteria], ["project_math", files.project_math], ["project_language", files.project_language], ["project_science", files.project_science], ["project_english", files.project_english]];
    ...
    {view === "adaptacion" && <section className="workspace">
      ...
      <div className="form-shell"><aside><span>01</span><strong>Datos personales</strong><i /><span>02</span><strong>Documentación</strong><i /><span>03</span><strong>Generar</strong></aside><div className="form-content">
        ...
        <div className="form-section"><span className="step-label">02 · DOCUMENTACIÓN Y MATERIALES</span><h2>Comparte el contexto pedagógico</h2><p className="section-help">Puedes arrastrar archivos o seleccionarlos. Formatos admitidos: PDF, Word, PowerPoint e imágenes.</p>
          ...
          <UploadBox id="criterios" eyebrow="PRIORIDAD DE APRENDIZAJE Y EVALUACIÓN" title="Selección de criterios por unidad" description="Añade el documento donde hayas marcado qué criterios quieres trabajar y evaluar en cada UDI. Se usarán para crear actividades, pruebas y productos finales." files={files.criterios} onFiles={addFiles} optional />
