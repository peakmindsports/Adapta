  ...
  return <main>
    ...
    {view === "adaptacion" && <section className="workspace">
      ...
      <div className="form-shell"><aside><span>01</span><strong>Datos del alumno</strong><i /><span>02</span><strong>Documentación</strong><i /><span>03</span><strong>Generar</strong></aside><div className="form-content">
        ...
        <div className="student-context"><span className="step-label">03 · CONTEXTO PERSONAL Y EDUCATIVO</span><h2>Lo que ayuda a este alumno a aprender</h2><p>Incluye solo información relevante para la intervención educativa. Describe situaciones observables y evita datos innecesarios de terceros.</p><div className="context-grid"><div className="notes-field"><label htmlFor="strengths">Fortalezas, intereses y motivadores <span>Opcional</span></label><textarea id="strengths" value={strengths} onChange={(e) => setStrengths(e.target.value)} placeholder="Qué le interesa, en qué destaca, qué actividades le motivan..." /></div><div className="notes-field"><label htmlFor="classroom-context">Situaciones en el aula y necesidades de apoyo <span>Opcional</span></label><textarea id="classroom-context" value={classroomContext} onChange={(e) => setClassroomContext(e.target.value)} placeholder="Atención, frustración, relación con iguales, autonomía, comunicación, desencadenantes observables..." /></div><div className="notes-field"><label htmlFor="family-context">Contexto familiar relevante <span>Opcional</span></label><textarea id="family-context" value={familyContext} onChange={(e) => setFamilyContext(e.target.value)} placeholder="Rutinas, disponibilidad de apoyo, idiomas familiares, coordinación necesaria..." /></div><div className="notes-field"><label htmlFor="effective-supports">Estrategias que funcionan y situaciones a evitar <span>Opcional</span></label><textarea id="effective-supports" value={effectiveSupports} onChange={(e) => setEffectiveSupports(e.target.value)} placeholder="Anticipación, descansos, apoyos visuales, refuerzo, agrupamientos, ajustes del entorno..." /></div></div></div><div className="notes-field"><label htmlFor="priorities">Otras indicaciones para la adaptación <span>Opcional</span></label><textarea id="priorities" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Ej. Priorizar actividades manipulativas, reducir la carga de escritura, mantener el tema de la unidad..." /></div>{notice && <div className="success-note">{notice}</div>}{result && <ResultPanel result={result} jobId={activeJob} />}
        <PhraseLibrary onPick={(category, phrase) => { if (category === "strengths") setStrengths((value) => value ? `${value} ${phrase}` : phrase); if (category === "classroom") setClassroomContext((value) => value ? `${value} ${phrase}` : phrase); if (category === "family") setFamilyContext((value) => value ? `${value} ${phrase}` : phrase); if (category === "supports") setEffectiveSupports((value) => value ? `${value} ${phrase}` : phrase); }} />
    ...
    {showHistory && <div className="history-overlay" onClick={() => setShowHistory(false)}><aside className="history-panel" onClick={(e) => e.stopPropagation()}><button className="history-close" onClick={() => setShowHistory(false)}>×</button><span className="section-kicker blue-ink">MI ESPACIO</span><h2>Trabajos guardados</h2><p>Consulta y descarga tus últimas propuestas.</p><div className="history-list">{history.length ? history.map((job) => <article key={job.id}><div><span>{job.kind === "adaptation" ? "ADAPTACIÓN" : "PROYECTO"}</span><strong>{job.title}</strong><small>{new Date(job.createdAt).toLocaleDateString("es-ES")} · {job.status === "completed" ? "Completado" : job.status}</small></div>{job.status === "completed" && <div className="history-downloads"><a href={`/api/jobs/${job.id}/download?format=pdf`}>PDF</a><a href={`/api/jobs/${job.id}/download?format=docx`}>Word</a></div>}</article>) : <div className="empty-history">Todavía no has generado ninguna propuesta.</div>}</div></aside></div>}
...
function ResultPanel({ result, jobId }: { result: string; jobId: string }) {
  return <section className="result-panel"><div><span>PROPUESTA GENERADA</span><h2>Tu libro está listo</h2></div><div className="result-downloads"><a href={`/api/jobs/${jobId}/download?format=pdf`}>Descargar PDF ↓</a><a href={`/api/jobs/${jobId}/download?format=docx`}>Descargar Word ↓</a></div><pre>{result}</pre></section>;
}
...
function PhraseLibrary({ onPick }: { onPick: (category: string, phrase: string) => void }) {
  const [open, setOpen] = useState(false); const [category, setCategory] = useState("strengths"); const [saved, setSaved] = useState<Array<{ id: string; category: string; phrase: string }>>([]); const [draft, setDraft] = useState("");
  useEffect(() => { fetch("/api/context-phrases").then((response) => response.ok ? response.json() : { phrases: [] }).then((data) => setSaved(data.phrases || [])).catch(() => {}); }, []);
  const save = async () => { const phrase = draft.trim(); if (!phrase) return; const response = await fetch("/api/context-phrases", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ category, phrase }) }); if (response.ok) { const data = await response.json(); setSaved((items) => [...items, data.phrase]); onPick(category, phrase); setDraft(""); } };
  const remove = async (id: string) => { const response = await fetch(`/api/context-phrases?id=${encodeURIComponent(id)}`, { method: "DELETE" }); if (response.ok) setSaved((items) => items.filter((item) => item.id !== id)); };
  const choices = [...phraseDefaults[category].map((phrase) => ({ id: "", category, phrase })), ...saved.filter((item) => item.category === category)];
  return <section className="phrase-library"><button type="button" className="phrase-toggle" onClick={() => setOpen(!open)}>＋ Seleccionar o crear frases tipo</button>{open && <div className="phrase-box"><div className="phrase-tabs">{Object.entries(phraseLabels).map(([key, label]) => <button type="button" key={key} className={category === key ? "active" : ""} onClick={() => setCategory(key)}>{label}</button>)}</div><p>Pulsa una frase para añadirla al campo correspondiente.</p><div className="phrase-chips">{choices.map((item, index) => <span key={`${item.id}-${index}`}><button type="button" onClick={() => onPick(category, item.phrase)}>{item.phrase}</button>{item.id && <button type="button" className="delete-phrase" aria-label="Eliminar frase guardada" onClick={() => remove(item.id)}>×</button>}</span>)}</div><div className="new-phrase"><input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Crea una frase para reutilizarla..." maxLength={300} /><button type="button" onClick={save}>Guardar y añadir</button></div></div>}</section>;
}
