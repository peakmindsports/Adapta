  ...
  return <main>
    ...
    {view === "adaptacion" && <section className="workspace">
      ...
      <div className="form-shell"><aside><span>01</span><strong>Datos personales</strong><i /><span>02</span><strong>Documentación</strong><i /><span>03</span><strong>Generar</strong></aside><div className="form-content">
        ...
        <PhraseCreator />
        <div className="form-footer"><p><strong>Libro anual completo</strong><br />Cada UDI se convertirá en un capítulo adaptado y todo quedará unido en un documento.</p><button className={`primary ${processing ? "progress-primary" : ""}`} disabled={processing} onClick={() => generate("adaptation")}>{processing ? <ButtonProgress value={progress} label="Creando el libro por unidades…" detail={progressLabel} /> : <>Generar libro adaptado <span>✦</span></>}</button></div>
      </div></div>
    </section>}
...
function ButtonProgress({ value, label, detail }: { value: number; label: string; detail: string }) {
  return <span className="button-progress" aria-live="polite" aria-label={`${label} ${value}%`}><span className="button-progress-heading"><strong>{label}</strong><b>{value}%</b></span><span className="button-progress-track"><i style={{ width: `${value}%` }} /></span><small>{detail || (value < 53 ? "Subiendo documentos…" : value < 95 ? "Preparando el contenido…" : "Terminando…")}</small></span>;
}
