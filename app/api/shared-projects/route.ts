...
export async function GET(request: Request) {
  ...
  const rows = await runtime().DB.prepare("SELECT jobs.id, jobs.title, jobs.kind, jobs.current_course AS currentCourse, jobs.target_course AS targetCourse, jobs.subject, jobs.academic_year AS academicYear, jobs.teacher_name AS teacherName, jobs.owner_email AS ownerEmail, jobs.shared_at AS sharedAt, CASE WHEN reads.project_id IS NULL THEN 0 ELSE 1 END AS isRead FROM jobs LEFT JOIN shared_project_reads AS reads ON reads.project_id = jobs.id AND reads.owner_email = ? WHERE jobs.shared_at IS NOT NULL AND jobs.status = 'completed' ORDER BY jobs.shared_at DESC").bind(owner).all();
  return Response.json({ projects: rows.results.map((resource: any) => ({ ...resource, title: sharedTitle(resource), isMine: resource.ownerEmail === owner, isRead: Boolean(resource.isRead) })) });
}
...
export async function PATCH(request: Request) {
  ...
  const resource = await runtime().DB.prepare("SELECT id FROM jobs WHERE id = ? AND owner_email <> ? AND shared_at IS NOT NULL AND status = 'completed'").bind(body.projectId, owner).first();
...
export async function POST(request: Request) {
  await ensureSchema(); const owner = await activeOwnerFrom(request); if (!owner) return authenticationError(); const body = await request.json() as { projectId?: string };
  if (!body.projectId) return jsonError("Selecciona un recurso.");
  const source = await runtime().DB.prepare("SELECT kind, title, student_name, current_course AS currentCourse, target_course AS targetCourse, subject, academic_year, teacher_name, result FROM jobs WHERE id = ? AND shared_at IS NOT NULL AND status = 'completed'").bind(body.projectId).first<Record<string, any>>();
