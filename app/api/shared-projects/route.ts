...
export async function GET(request: Request) {
  await ensureSchema(); const owner = ownerFrom(request); if (!owner) return authenticationError();
...
export async function PATCH(request: Request) {
  await ensureSchema(); const owner = ownerFrom(request); if (!owner) return authenticationError(); const body = await request.json() as { projectId?: string };
