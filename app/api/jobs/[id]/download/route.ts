...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const url = new URL(request.url); const format = url.searchParams.get("format") === "docx" ? "docx" : "pdf"; const requested = (url.searchParams.get("units") || "").split(",").map(Number).filter((value) => Number.isInteger(value) && value > 0); const scopeValue = url.searchParams.get("scope"); const scope: DownloadScope = scopeValue === "student" || scopeValue === "teacher" ? scopeValue : "all"; const repaired = repairIndex(job.result); const scoped = resourceForScope(repaired, requested, scope); const result = studentSafeMarkdown(scoped); const unitSuffix = requested.length ? `-UDI-${requested.join("-")}` : ""; const scopeSuffix = scope === "student" ? "-Alumnado" : scope === "teacher" ? "-Docente" : "-Completo"; const filename = `${safeFilename(job.title)}${unitSuffix}${scopeSuffix}`; const images = await sourceImages(id, owner, requested, requiredImages(result)); const fallbackCover = async (): Promise<SourceImage> => { const fallback = await fetch(new URL("/resource-cover.png", request.url)); return { bytes: new Uint8Array(await fallback.arrayBuffer()), width: 1024, height: 1536, type: "png" }; }; let coverImage = images.find((image): image is SourceImage => Boolean(image)) || await fallbackCover(); const cover = { subject: job.subject, student: job.student, course: job.course, academicYear: job.academicYear };
  if (format === "docx") { let blob: Blob; try { blob = await makeWord(job.title, result, images, cover, coverImage); } catch { coverImage = await fallbackCover(); blob = await makeWord(job.title, result, images.map(() => null), cover, coverImage); } return new Response(blob, { headers: { "Content-Type": "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "Content-Disposition": `attachment; filename="${filename}.docx"`, "Cache-Control": "private, no-store" } }); }
  let bytes: Uint8Array; try { bytes = await makePdf(job.title, result, images, cover, coverImage); } catch { coverImage = await fallbackCover(); bytes = await makePdf(job.title, result, images.map(() => null), cover, coverImage); } const body = bytes.slice().buffer as ArrayBuffer;  return new Response(body, { headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${filename}.pdf"`, "Cache-Control": "private, no-store" } });
}
