...
function normalizeInitialAssessmentLayout(markdown: string) {
  const separate = (value: string, prefix: string) => value.split(/(?<=[.!?])\s+(?=[A-ZÁÉÍÓÚÜÑ¿¡])/).map((item) => item.trim()).filter(Boolean).map((item) => `${prefix} ${item}`).join("\n");
  return markdown
    .replace(/(Marca con una X[^\n:]*:\s*)\n\s*([^\n|#][^\n]*)/gi, (_, instruction, options) => `${instruction}\n\n${separate(options, "- [ ]")}`)
    .replace(/(Ordena (?:los )?hechos[^\n:]*:\s*)\n\s*([^\n|#][^\n]*)/gi, (_, instruction, facts) => `${instruction}\n\n${separate(facts, "- ____")}`)
    .replace(/(^|\n)(?:\s*---\s*\n+)?\s*(?:#{1,6}\s*)?Rúbrica analítica\s*(?=\n)/gim, "$1\n---\n\n### Rúbrica analítica")
    .replace(/\n{4,}/g, "\n\n");
}

function paginateInitialAssessment(markdown: string) {
  markdown = normalizeInitialAssessmentLayout(markdown);
  const markerIndex = markdown.search(/<!--\s*INICIO_DOCENTE\s*-->/i);
  const student = markerIndex >= 0 ? markdown.slice(0, markerIndex) : markdown;
  const teacher = markerIndex >= 0 ? markdown.slice(markerIndex) : "";
  const paginated = student.replace(/(^|\n)(?!---\s*\n)(#{2,3}\s+(?:Prueba|Actividad|Tarea)\b[^\n]*)/gi, "$1---\n\n$2");
  return paginated + teacher;
}
...
function downloadableResult(result: string) {
  const clean = String(result || "").trim();
  const words = clean.split(/\s+/).filter(Boolean).length;
  return words >= 500 && !/necesito que (?:adjuntes|facilites|pegues)|archivo no (?:está|esta) disponible|cuando lo facilites|no puedo (?:elaborar|garantizar|continuar)|falta(?: el)? (?:archivo|documento)/i.test(clean);
}
