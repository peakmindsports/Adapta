...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  let bytes: Uint8Array; try { bytes = await makePdf(job.title, result, images, cover, coverImage); } catch { coverImage = fallbackCover(); bytes = await makePdf(job.title, result, images.map(() => null), cover, coverImage); } const body = bytes.slice().buffer as ArrayBuffer;  return new Response(body, { headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${filename}.pdf"`, "Cache-Control": "private, no-store" } });
}
