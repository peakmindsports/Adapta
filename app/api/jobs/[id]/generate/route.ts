  ...
  try {
    ...
    if (job.kind === "adaptation") {
      ...
      for (const file of contextFiles) {
        contextSummaries.push(await callModel([{ type: "input_text", text: `Analiza exclusivamente este documento (${file.category}: ${file.filename}). Extrae de forma compacta la información útil por unidad. Si es una selección curricular, conserva literalmente todos los criterios y todos los saberes seleccionados, incluidos códigos, descriptores, áreas y UDI asociadas, porque representan la prioridad del equipo docente. Si es programación, recoge objetivos, saberes, competencias y criterios; si es informe, medidas y apoyos; si es material de nivel, formato, lenguaje y dificultad. No diagnostiques ni inventes datos.` }, { type: "input_file", file_id: file.id }], file.category.includes("criterios") ? 3500 : 2200));
      }
