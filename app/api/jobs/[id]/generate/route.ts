...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (job.kind === "project") {
      const subjectNames: Record<string, string> = { project_math: "Matemáticas", project_language: "Lengua", project_science: "Conocimiento del Medio", project_english: "Inglés", proyecto: "Área sin clasificar" };
      content[0].text += `\n\nDOCUMENTOS CLASIFICADOS POR EL DOCENTE:\n${preparedFiles.map((file) => `- ${subjectNames[file.category] || file.category}: ${file.filename}`).join("\n")}\nRespeta esta clasificación al extraer programación, criterios y evidencias.`;
    }
