...
export async function POST(request: Request) {
  ...
  if (!body.kind || !["adaptation", "reinforcement", "project", "project_adaptation", "initial_assessment"].includes(body.kind)) return jsonError("Tipo de trabajo no válido.");
...
export async function DELETE(request: Request) {
  await ensureSchema();
  const owner = await activeOwnerFrom(request); if (!owner) return authenticationError(); const { DB, FILES } = runtime();
