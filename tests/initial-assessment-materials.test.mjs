...
test("renders printable visual sheets and breaks oversized words",async()=>{const route=await read("app/api/jobs/[id]/download/route.ts");assert.match(route,/type: "visual"/);assert.match(route,/const visual = line\.match/);assert.match(route,/block\.type === "visual"/);assert.match(route,/font\.widthOfTextAtSize\(word, size\) > maxWidth/);assert.match(route,/paginateInitialAssessment/);assert.match(route,/Prueba\|Actividad\|Tarea/);assert.match(route,/downloadableResult/);assert.match(route,/documento quedó incompleto/);});
